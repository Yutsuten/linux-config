function process_media --description 'Process media (photos and music) using its metadata'
    ...
    for music in *.mp3 *.m4a *.ogg *.opus *.flac
        ...
        ffprobe $file 2>&1 | sed -e "s/'/_/g" -e 's/"//g' > $ffprobe_out
        ...
        set ext    (echo $file | sed -nE 's/^.*\.([a-zA-Z0-9]{2,4})$/\1/p')

        mkdir -p "$album"
        mv $file "$album"/(printf '%02d' $disc)_(printf '%02d' $track)_"$title".$ext
        rm $ffprobe_out
    end
    return 0
end
